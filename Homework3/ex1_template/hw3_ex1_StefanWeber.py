...
def edge_map(img, sigma = 3):
    ...
    img_tmp = color.rgb2gray(img)
    edges = feature.canny(img_tmp, sigma)
...
def fit_line(points):
    ...
    if points[0,0]-points[1,0] == 0:
        m = (points[0,1]-points[1,1]) / (points[0,0]-points[1,0] + sys.float_info.epsilon)     
    else:
...
def point_to_line_dist(m, c, x0, y0):
    ...
    dist = np.abs(m * x0 - y0 + c) / np.sqrt(m ** 2 + 1)
