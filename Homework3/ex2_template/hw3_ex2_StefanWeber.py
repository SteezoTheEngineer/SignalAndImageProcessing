...
texture_region = color.rgb2gray(texture_region)
...
while (len(fill_indices[0])  > 0):
    ...
    while(len(edge_pixels[0]) > 0):
        ...
        index_center = np.random.randint(0,len(edge_pixels[0]))
        patch_center_i = edge_pixels[0][index_center]
        patch_center_j = edge_pixels[1][index_center]
        ...
        selected_center_i, selected_center_j = np.unravel_index(np.argmin(ssd_img, axis=None), ssd_img.shape) 
        selected_center_i += patch_half_size
        selected_center_j += patch_half_size
